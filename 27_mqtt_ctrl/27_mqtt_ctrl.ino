...
const char* MQTT_CTRL_TOPIC = "elsie/class305/ctrl/led/+";
...
bool setLedState(int pin, const char* device, const char* state) {
  bool isOn;
  ...
  commandDisplay = String(device) + (isOn ? " ON" : " OFF");
...
void mqttCallback(char* topic, byte* payload, unsigned int length) {
  const char* topicPrefix = "elsie/class305/ctrl/led/";
  const size_t topicPrefixLength = strlen(topicPrefix);
  if (strncmp(topic, topicPrefix, topicPrefixLength) != 0 || strlen(topic) != topicPrefixLength + 1) {
    return;
  }
  ...
  switch (topic[topicPrefixLength]) {
    case 'g': ledPin = LED_G_PIN; ledName = "GREEN"; legacyKey = "gled"; break;
    case 'y': ledPin = LED_Y_PIN; ledName = "YELLOW"; legacyKey = "yled"; break;
    case 'r': ledPin = LED_R_PIN; ledName = "RED"; legacyKey = "rled"; break;
    default:
      Serial.println("Unknown LED topic");
      return;
  }
  ...
  if (deserializeJson(document, payload, length) == DeserializationError::Ok) {
    if (document[legacyKey].is<const char*>()) {
      state = document[legacyKey];
    } else if (document["state"].is<const char*>()) {
  ...
  char textState[8];
  if (state == nullptr) {
    if (length == 0 || length >= sizeof(textState)) {
      Serial.println("Invalid LED payload");
      return;
    }
    memcpy(textState, payload, length);
    textState[length] = '\0';
    state = textState;
  }

  setLedState(ledPin, ledName, state);
}

void drawCommandScreen() {
  oled.clearBuffer();
  oled.setFont(u8g2_font_6x10_tf);
  oled.drawStr(30, 12, "MQTT CONTROL");
  oled.drawHLine(0, 16, 128);
  oled.setFont(u8g2_font_logisoso16_tf);
  int textWidth = oled.getStrWidth(commandDisplay.c_str());
  oled.drawStr((128 - textWidth) / 2, 49, commandDisplay.c_str());
  oled.setFont(u8g2_font_6x10_tf);
  oled.drawStr(35, 62, "COMMAND RECEIVED");
  oled.sendBuffer();
}
void connectWiFi() {
  WiFi.mode(WIFI_STA);
...
bool mqttConnect() {
  ...
  if (connected) mqttClient.subscribe(MQTT_CTRL_TOPIC);
...
void loop() {
  ...
  if (millis() < commandDisplayUntil) drawCommandScreen();
  else drawScreen(temperature, humidity, lightPercent, dhtValid);
