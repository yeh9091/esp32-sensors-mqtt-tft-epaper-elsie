...
const int GLED=15,YLED=2,RLED=4;
...
WiFiClient net; PubSubClient mqtt(net); unsigned long lastPublish=0,lastTry=0,lastClock=0;
...
void drawStaticUI(){fastFill(0,0,W,H,BG);fastFill(0,0,W,31,BLUE);tft.setTextSize(2);tft.setTextColor(WHITE);tft.setCursor(12,7);tft.print("ROOM MONITOR");tft.fillCircle(158,15,6,GREEN);tft.drawCircle(158,15,8,WHITE);drawStatus();drawCard(47,"TEMPERATURE",0,"C",ORANGE,0,false);drawCard(101,"HUMIDITY",0,"%",CYAN,1,false);drawCard(155,"BRIGHTNESS",0,"%",YELLOW,2,true);tft.setTextSize(1);tft.setTextColor(MUTED);tft.setCursor(35,211);tft.print("MQTT CTRL / 10 SEC");}
void drawClock(){
  struct tm timeinfo; char text[20];
  if(getLocalTime(&timeinfo,50)) strftime(text,sizeof(text),"TW %H:%M:%S",&timeinfo);
  else snprintf(text,sizeof(text),"TIME --:--:--");
  fastFill(0,207,W,13,BG); tft.setTextSize(1); tft.setTextColor(MUTED); tft.setCursor(50,210); tft.print(text);
}
...
void applyLed(const char* key, JsonVariant value, int pin){if(!value.is<const char*>())return;const char* state=value.as<const char*>();if(strcmp(state,"on")==0)digitalWrite(pin,HIGH);else if(strcmp(state,"off")==0)digitalWrite(pin,LOW);Serial.printf("%s=%s\n",key,state);}
void mqttCallback(char* topic, byte* payload, unsigned int length){if(strcmp(topic,CTRL_TOPIC)!=0)return;StaticJsonDocument<192> doc;DeserializationError err=deserializeJson(doc,payload,length);if(err){Serial.print("JSON error: ");Serial.println(err.c_str());return;}applyLed("gled",doc["gled"],GLED);applyLed("yled",doc["yled"],YLED);applyLed("rled",doc["rled"],RLED);}
void connectWiFi(){WiFi.mode(WIFI_STA);WiFi.begin(WIFI_SSID,WIFI_PASSWORD);for(int i=0;i<30&&WiFi.status()!=WL_CONNECTED;i++){drawStatus();delay(500);}drawStatus();Serial.println(WiFi.status()==WL_CONNECTED?"WiFi connected":"WiFi failed");}
void connectMQTT(){if(WiFi.status()!=WL_CONNECTED)return;String id="ESP32-CTRL-"+String((uint32_t)(ESP.getEfuseMac()&0xFFFFFFFF),HEX);mqtt.setServer(MQTT_HOST,MQTT_PORT);mqtt.setCallback(mqttCallback);if(mqtt.connect(id.c_str())){mqtt.subscribe(CTRL_TOPIC);Serial.println("MQTT connected/subscribed");}else Serial.printf("MQTT failed state=%d\n",mqtt.state());drawStatus();}
void publishData(int t,int h,int l,bool valid){if(!mqtt.connected())return;char json[80];snprintf(json,sizeof(json),"{\"temp\":%d,\"humi\":%d,\"light\":%d}",valid?t:0,valid?h:0,l);mqtt.publish(DATA_TOPIC,json);Serial.print("MQTT data: ");Serial.println(json);}
void setup(){Serial.begin(115200);analogReadResolution(12);analogSetPinAttenuation(LIGHT_PIN,ADC_11db);pinMode(GLED,OUTPUT);pinMode(YLED,OUTPUT);pinMode(RLED,OUTPUT);digitalWrite(GLED,LOW);digitalWrite(YLED,LOW);digitalWrite(RLED,LOW);pinMode(TFT_CS,OUTPUT);pinMode(TFT_DC,OUTPUT);pinMode(TFT_RST,OUTPUT);digitalWrite(TFT_CS,HIGH);SPI.begin(TFT_SCK,-1,TFT_MOSI,TFT_CS);SPI.beginTransaction(SPISettings(8000000,MSBFIRST,SPI_MODE0));iliBegin();tft.setRotation(2);drawStaticUI();drawClock();SPI.endTransaction();Wire.begin(OLED_SDA,OLED_SCL);oled.begin();drawOled(0,0,0,false);connectWiFi();configTime(8*3600,0,"pool.ntp.org","time.nist.gov","time.google.com");connectMQTT();}
void loop(){if(WiFi.status()!=WL_CONNECTED)connectWiFi();if(!mqtt.connected()&&millis()-lastTry>5000){lastTry=millis();connectMQTT();}mqtt.loop();int t=0,h=0,l=constrain(map(analogRead(LIGHT_PIN),0,4095,0,100),0,100);bool valid=readDHT(t,h);SPI.beginTransaction(SPISettings(8000000,MSBFIRST,SPI_MODE0));drawValues(t,h,l,valid);if(millis()-lastClock>=10000){lastClock=millis();drawClock();}SPI.endTransaction();drawOled(t,h,l,valid);if(millis()-lastPublish>=10000){lastPublish=millis();publishData(t,h,l,valid);}delay(1000);}
